...
if(isset($_POST['search'])){
	...
	if($coin_type!="ALL")
		$filter.=" and wallet LIKE '%$coin_type%'";
	if($user_id!="all")
	{
...
$total_rows = mysqli_num_rows(mysqli_query($conn, "(SELECT tranfer.details,tranfer.type_method,tranfer.invoice_no,tranfer.id,users.name,users.mobile_number,tranfer.amount,tranfer.wallet,tranfer.created_on,'Send' AS tx_type FROM tranfer,users WHERE tranfer.sender_id=".$_SESSION['login']." AND users.id=tranfer.receiver_id  $filter) UNION (SELECT tranfer.details,tranfer.type_method,tranfer.invoice_no,tranfer.id,users.name,users.mobile_number,tranfer.amount,tranfer.wallet,tranfer.created_on,'Receives' AS tx_type FROM tranfer,users WHERE tranfer.receiver_id=".$_SESSION['login']." AND users.id=tranfer.sender_id  $filter)"));
$total_page_num = ceil($total_rows / $limit);
$start = ($page - 1) * $limit;
$end = $page * $limit;
    $query="(SELECT tranfer.details,tranfer.type_method,tranfer.invoice_no,tranfer.id,users.name,users.mobile_number,tranfer.amount,tranfer.wallet,tranfer.created_on,'Send' AS tx_type FROM tranfer,users WHERE tranfer.sender_id=".$_SESSION['login']." AND users.id=tranfer.receiver_id  $filter) UNION (SELECT tranfer.details,tranfer.type_method,tranfer.invoice_no,tranfer.id,users.name,users.mobile_number,tranfer.amount,tranfer.wallet,tranfer.created_on,'Receives' AS tx_type FROM tranfer,users WHERE tranfer.receiver_id=".$_SESSION['login']." AND users.id=tranfer.sender_id  $filter) ORDER BY id DESC LIMIT $start,$end";
