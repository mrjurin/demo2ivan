...
$m_id=$_GET['m_id'];
$profile_data = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM users WHERE id='".$m_id."'"));
$invitation_id=$profile_data['referral_id'];
function gw_send_sms($user,$pass,$sms_from,$sms_to,$sms_msg){ 
   // $sms_msg=strToHex($sms_msg);          
    // $query_string = "api.aspx?apiusername=".$user."&apipassword=".$pass;
    // $query_string .= "&senderid=".rawurlencode($sms_from)."&mobileno=".rawurlencode($sms_to);
    // $query_string .= "&message=".urlencode(($sms_msg)) . "&languagetype=1";        
    // $url = "http://gateway.onewaysms.com.au:10001/".$query_string;  
	    
    $url = "http://gateway.onewaysms.com.au:10001/api.aspx?apiusername=".$user."&apipassword=".$pass."&senderid=".$sms_from."&mobileno=".$sms_to."&message=".urlencode($sms_msg)."&languagetype=1";  
	  
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    $curl_scraped_page = curl_exec($ch);
    curl_close($ch);
    $msg  = $curl_scraped_page;
   // die;
...
if(isset($_POST['signup']))
{
	...
	if($error == "")
	{
	    ...
	    $otp = generatePIN();
	     $q="INSERT INTO users SET login_token='$login_token',dob='$dob', isLocked='1', name='$name',user_roles='$user_role', password='$password', joined='".time()."', verification_code='$code', referral_id='$ref',referred_by='$reffered_by',security_answer= '$security',security_questions= '$questions',fund_password='$password',email='$email',mobile_number='$cm'";
		
	      mysqli_query($conn,$q); 
		$user_id = mysqli_insert_id($conn);
			
		 // $current_url = "https://".$_SERVER['HTTP_HOST'].""."/login.php";
		 $current_url="https://www.koofamilies.com/structure_merchant.php?merchant_id=".$m_id."&l=".$login_token;
		// die;
		...
		// gw_send_sms("APIHKXVL33N5E", "APIHKXVL33N5EHKXVL", "9787136232", "$cm", "$otp is your otp code for Koo Families $sms_msg");
	   	gw_send_sms("APIHKXVL33N5E", "APIHKXVL33N5EHKXVL", "9787136232", "$cm", "Verify Your Account on koofamilies $current_url");
		...
		<p style='text-align:center'><a href='$current_url'>Verify</a></p>
		</body>
		</html>
		";
        ...
        //mysqli_query($conn,"UPDATE users set isLocked='1' where id='$user_id'");
	}
...
-->
	...
	if($about['image']){ ?>
        <img src="<?php echo $site_url; ?>/images/about_images/<?php echo $about['image'];  ?>" width="170px" height="100px" class="logo_main">
	<?php } else { ?>
                ...
                <div class="sap_tabs">
                    <div id="horizontalTab" style="display: block; width: 100%; margin: 0px;">
                        <ul class="resp-tabs-list">
                            <li class="resp-tab-item" aria-controls="tab_item-1" role="tab"><span>Add Member</span></li>
                            <div class="clear"> </div>
                        </ul>
									...
									<div class="login-top sign-top">
										...
										<div >
											...
											<div class="form-group form-check">
															...
															<a  target="_blank" href="uploads/Termskoofood.pdf">Terms & Conditions </a> 
															<?php if($_GET['invitation_id']=="Life Ricette Cafe Patisserie 5d706a7ce199f"){ ?>
															, <a  target="_blank" href="uploads/lifeterms.pdf">Membership Agreement</a>
															<?php } ?>
															 & <a  target="_blank" href="uploads/privacy_agreement.pdf">Privacy Policy</a></label>
											</div>
